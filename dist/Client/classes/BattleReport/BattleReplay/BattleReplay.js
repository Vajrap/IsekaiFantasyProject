...
class BattleReplay {
    ...
    applyActorSkillEffects(element, effects) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!element)
                return;
            ...
            for (const effect of effects) {
                ...
                yield delay(effectDuration);
                element.classList.remove(`skill-effect-${effect}`);
            }
        });
    }
    applyTargetSkillEffects(targets, effects) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    updateCharacterStats(currentTurn) {
        const updateStatsForCharacter = (character) => {
            ...
            if (charContainer) {
                ...
                if (charStatsHP) {
                    charStatsHP.textContent = `🔴 ${character.currentHP}/${character.maxHP}`;
                }
                const charStatsMP = charContainer.querySelector('.battleReplay-charStats-Element:nth-child(2)');
                if (charStatsMP) {
                    charStatsMP.textContent = `🔵 ${character.currentMP}/${character.maxMP}`;
                }
                const charStatsSP = charContainer.querySelector('.battleReplay-charStats-Element:nth-child(3)');
                if (charStatsSP) {
                    charStatsSP.textContent = `🟡 ${character.currentSP}/${character.maxSP}`;
                }
