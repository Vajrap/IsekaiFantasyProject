...
async function main() {
  ...
  for (const attribute of Object.keys(character.status.attributes) as Array<
    keyof typeof character.status.attributes
  >) {
  ...
  while (battleWon > 0) {
    for (const attribute in character.status.attributes) {
      character.train(attribute as CharacterStatusEnum);
    }
    battleWon--;
