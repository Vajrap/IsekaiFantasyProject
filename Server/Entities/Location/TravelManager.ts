...
export class travelingPartyImpl implements travelingParty {
  ...
  checkIfArrivingNextLocation(): boolean {
    ...
    return (
      ...
      this.currentLocation.calculateDistanceTo(nextLocation)
    );
  }
}
...
export class TravelManager {
  ...
  async allTravel(day: DayOfWeek, phase: TimeOfDay) {
    ...
    for (const travelingParty in this.travelingParties) {
      if (
        this.travelingParties[travelingParty].party.actionSequence[day][phase]
          .type === LocationActionEnum.Travel
      ) {
        travelingParties.push(this.travelingParties[travelingParty]);
      }
    }
  ...
  updateDistace(party: travelingParty, isEventHappened: boolean = false) {
    ...
    const { travelSpeed, averageLuckModifier } =
      getTravelSpeedAndAverageLuckModifier(party);
  ...
  async _executeBattleEvent(
    ...
    event_battle(travelingParty.party, enemyParty, travelingParty.party.location, BattleType.Random);
    return true;
  }
}
