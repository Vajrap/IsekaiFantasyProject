import { gameEvent_houseRest, gameEvent_innRest, gameEvent_rest } from "../../Game/GameEvent/GameEvent";
...
LocationActionRepository.moveOut = new LocationAction(
    ...
    (location: GameLocation, party: Party) => {
        for (const character in party.characters) {
            if (character !== null) {
                location.partyMoveOut(party)
            }
        }
    }
);
...
LocationActionRepository.rest = new LocationAction(
    ...
    (location: GameLocation, party: Party, characterID: string) => {
        const highestLuck = party.characters.reduce((acc, char) => char != null && char != 'none' && acc < char.attribute('luck') ? char.attribute('luck') : acc, 0)
        const region = getRegionFromName(location.region)
        let randomEvent = region.getRandomEvent('rest', StatMod.value(highestLuck))
        if (randomEvent) {
            // randomEvent.execute({ party: party, actor: characterID })
            // execute different events based on the randomEvent
        }
