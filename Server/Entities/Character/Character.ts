...
export class Character {
	...
	*/
	...
	receiveBuff(
		...
		if (this.buffsAndDebuffs.cursed > 0 && this.buffsAndDebuffs.bless < this.buffsAndDebuffs.cursed) {
		...
		if (buff === BuffsAndDebuffsEnum.bless && this.type === CharacterType.undead) {
			return {result: false, message: "Undead cannot be blessed."}
		};
		if (buff === BuffsAndDebuffsEnum.rejuvenate && this.type === CharacterType.undead) {
	...
	receiveDebuff(
		...
		if (this.buffsAndDebuffs.bless > 0 && this.buffsAndDebuffs.cursed < this.buffsAndDebuffs.bless) {
			const save = Dice.rollTwenty() + StatMod.value(this.status.luck())
			if (save > 10 - this.buffsAndDebuffs.bless) {
				return {result: false, message: "Debuff failed due to bless."};
			}
		}
		if (debuff === BuffsAndDebuffsEnum.bleed && this.type === CharacterType.undead) {
			return {result: false, message: "Undead cannot bleed."}
		};
		if (debuff === BuffsAndDebuffsEnum.poison && this.type === CharacterType.undead) {
			return {result: false, message: "Undead cannot be poisoned."}
		};
		if (debuff === BuffsAndDebuffsEnum.burn && this.buffsAndDebuffs.freeze > 0) { 
			this.buffsAndDebuffs.freeze = 0; 
			additionalMessage += `Freeze debuff removed.`;
		};
		if (debuff === BuffsAndDebuffsEnum.burn && this.buffsAndDebuffs.soaked > 0) {
			return {result: false, message: "Soaked target cannot be burned."}
		};
		if (debuff === BuffsAndDebuffsEnum.freeze && this.buffsAndDebuffs.burn > 0) {
			this.buffsAndDebuffs.burn = 0; 
			additionalMessage += `Burn debuff removed.`
		};
		if (debuff === BuffsAndDebuffsEnum.freeze && this.type === CharacterType.undead) {
			return {result: false, message: "Undead cannot be frozen."}
		};
		if (debuff === BuffsAndDebuffsEnum.soaked && this.buffsAndDebuffs.burn > 0) {
			this.buffsAndDebuffs.burn = 0; 
			additionalMessage += `Burn debuff removed.`
		};
