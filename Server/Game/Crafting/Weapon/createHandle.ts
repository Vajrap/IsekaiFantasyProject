...
export function createHandle(
    ...
    pHIT: number, pATK: number, pCRT: number, mHIT: number, mATK: number, mCRT: number, order: number, chaos: number, geo: number, water: number, air: number, fire: number, chiCold: number, chiWarm: number, chiHarmony: number, arcaneAptitude: number
} {
    let pHIT = 0;
    let pATK = 0;
    let pCRT = 0;
    ...
    for (let i = 0; i < bonusPoint; i++) {
        ...
        switch (traitResult) {
            case TraitResultEnum.pHIT:
                pHIT += bonusMagnitude;
                break;
            case TraitResultEnum.pATK:
                pATK += bonusMagnitude;
                break;
            case TraitResultEnum.pCRT:
                pCRT += bonusMagnitude;
                break;
            case TraitResultEnum.mHIT:
                mHIT += bonusMagnitude;
                break;
            case TraitResultEnum.mATK:
                mATK += bonusMagnitude;
                break;
            case TraitResultEnum.mCRT:
                mCRT += bonusMagnitude;
                break;
    ...
    return { pHIT, pATK, pCRT, mHIT, mATK, mCRT, order, chaos, geo, water, air, fire, chiCold, chiWarm, chiHarmony, arcaneAptitude };  
}