...
export class SkillCard {
    ...
    createFrontFace() {
        ...
        if (equipmentRequirements) frontFace.appendChild(equipmentRequirements);

        const consume = this.createSkillCardConsume();
        if (consume) frontFace.appendChild(consume);

        const produce = this.createSkillCardProduce();
        if (produce) frontFace.appendChild(produce);
    ...
    createSkillEquipmentRequirements(): HTMLDivElement | undefined {
    ...
    createSkillCardConsume(): HTMLDivElement | undefined {
        ...
        if (consumeValue === '') { return undefined };
    ...
    createSkillCardProduce(): HTMLDivElement | undefined {
        let produceValue = '';
